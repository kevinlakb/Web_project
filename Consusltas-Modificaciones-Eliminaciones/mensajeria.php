    ...
    <script language="javascript" type="text/javascript">  
        ...
        function valida(F) {  
            if( vacio(F.codigo.value) == false ) {  
                    alert("EL CAMPO ID_MENSAJERIA ESTA VÁCIO, DEBE DIGITAR UN VALOR.")  
                    return false  
            } else {  
...
<div id="con_men_eli" class="modal">
    <div class="content-modal" style="overflow-x: scroll;">
        <header>
        <?PHP
    $conn = connect_db();
    extract($_POST);
    $sql1="select * from mensajeria where idmensajeria='$codigo'";
    $result1 = pg_query($conn,$sql1);
?>
   <table> 
    <thead>
        <td colspan="5">Formulario de Mensajeria</td>
    </thead>
    <tr>
        <td>&nbsp;ID_Mensajeria&nbsp;</td><td>&nbsp;Nombres&nbsp;</td><td>&nbsp;Celular&nbsp;</td><td>&nbsp;Telefono&nbsp;</td><td>&nbsp;Correo&nbsp;</td>
    </tr>
<?PHP    
  ...
  pg_close($conn1); 
        ...
        <form action="mensajeria.php#con_men" method="post">
...
<div id="con_men_mod" class="modal">
    <div class="content-modal" style="overflow-x: scroll;">
        <header>
        <?PHP
        $conn = connect_db();
        ...
        <?PHP    
        echo "<form action='mensajeria.php#con_men' method='post'>";
        while($row=pg_fetch_array($result1)){ 
            $idmensajeria=$row["0"];
            $nombre=$row["1"];
            $celular=$row["2"];
            ...
            echo "<tr>";
            echo "<td >";
            echo "<input  type='text' name='idmensajeria' value='".$idmensajeria."' readonly>";
            echo "</td>";
            echo "<td >";
...
<div id="con_men" class="modal">
    <div class="content-modal">
                    ...
                    $conn = connect_db();
                    if($var=='eliminar'){
                    $sql2= "delete from mensajeria where idmensajeria='$idmensajeria'"; 
                    $result2 = pg_query($conn,$sql2);
                    pg_free_result($result2); 
                    echo "<p style='font-size: 18px; color: #3d3d3d; padding: 20px; font-family: 'bellford';'>SE HA ELIMINADO EL MENSAJERIA</p>";
                    }
                    if($var=='modificar'){
                    $sql3= "UPDATE mensajeria set nombre='".$nombre."', celular='".$celular."', telefono='".$telefono."', correo='".$correo."' where idmensajeria='".$idmensajeria."'";
